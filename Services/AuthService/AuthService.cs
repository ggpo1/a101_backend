...
namespace a101_backend.Services.AuthService
{
    public class AuthService : IAuthService
    {
        ...
        {
            User usr = MyDb.db.User.FirstOrDefault(elem => elem.UserName == user.Login);
            if (user.Password != usr.PasswordHash)
                return await Task.Run(() => new { User = new { UserID = usr.UserID, UserName = usr.UserName }, Status = UserAuthStatus.FAILED });
